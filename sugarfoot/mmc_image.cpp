...
void MMCImage::load_aliases(oop imodule, oop aliases_dict, number num_params) {
  ...
  for (int i = 0 ; it != end; it++, i++) {
    ...
    debug() << "alias name " << _mmobj->mm_symbol_cstr(_proc, alias_name)
...
void MMCImage::create_alias_getters(oop imodule, oop imod_dict,
  ...
  debug() << "Creating aliases: " << _mmobj->mm_dictionary_size(_proc, aliases_dict) << endl;
  ...
  for (int i = 0 ; it != end; it++, i++) {
    ...
    debug() << "alias dict has " << _mmobj->mm_dictionary_size(_proc, imod_dict) << endl;
  }
}
...
oop MMCImage::instantiate_module(oop module_arguments_list) {
  ...
  debug() << "imodule " << imodule << " name:" << _mmobj->mm_string_cstr(_proc, name) << " params:" << num_params
          << " aliases " << num_aliases

...
MMCFunction::MMCFunction(VM* vm, CoreImage* core_image, char* data, int data_size)
  : _vm(vm), _mmobj(vm->mmobj()), _core_image(core_image), _data_size(data_size), _data(data) {
}
...
void MMCFunction::load_header() {
  ...
  debug() << " ============ eval: ===========" << endl;
  debug() << "Header:magic: " << magic_number << " =?= " << MMCFunction::MAGIC_NUMBER << endl;
  ...
  debug() << "Header:cfun_addr: " << _cfun_addr << endl;
}
